...
5. **Program's explicit rules** — check for:
   - "Do not report cookie/header issues unless you demonstrate unauthorized access"
   - "Permissions granted in API are intentionally granted"
   - "Default creds in open source = expected behavior"
   - "Vulnerabilities in forked repos not eligible if present in upstream"
   - "API token in Authorization header is by design"
...
Health endpoints on internal APIs (standard practice, informational)
OpenAPI/Swagger spec publicly accessible (by design for public APIs)
Default IP filter 0.0.0.0/0 on new services (by design for cloud platforms)
```
