...
## PASSIVE SUBDOMAIN FALLBACK CHAIN (when crt.sh rate-limits AND no PD tools)

crt.sh frequently returns an HTML `ERROR!` page (rate-limit/DB timeout) instead of JSON, and a fresh box often has none of subfinder/amass/assetfinder installed. Don't stall — chain these no-auth HTTP sources, all dependency-free (curl + jq only):

```bash
DOMAINS="newrelic.com nr-data.net nr-ops.net"   # space-separated; covers multi-domain scope
> /tmp/subs.txt
for d in $DOMAINS; do
  # certspotter (cert transparency, JSON)
  curl -s "https://api.certspotter.com/v1/issuances?domain=$d&include_subdomains=true&expand=dns_names" -m 30 \
    | jq -r '.[].dns_names[]?' 2>/dev/null | sed 's/\*\.//g' >> /tmp/subs.txt
  # hackertarget (CSV: host,ip — free tier ~50 results/day per IP)
  curl -s "https://api.hackertarget.com/hostsearch/?q=$d" -m 30 | cut -d',' -f1 >> /tmp/subs.txt
  # rapiddns (HTML scrape, often the richest source when others are dry)
  curl -s "https://rapiddns.io/subdomain/$d?full=1" -m 30 \
    | grep -oP "(?<=<td>)[a-zA-Z0-9.-]+\.$d" >> /tmp/subs.txt
done
# clean + restrict to in-scope apex domains
grep -E "\.(newrelic\.com|nr-data\.net|nr-ops\.net)$" /tmp/subs.txt | sort -u > /tmp/subs_clean.txt
echo "Total: $(wc -l < /tmp/subs_clean.txt)"
```

In a real run this chain produced 119 subdomains for newrelic.com with zero from crt.sh (errored) and zero from certspotter — **rapiddns + hackertarget carried the whole thing**. Other no-auth sources to add if these are thin: `web.archive.org/cdx/search/cdx?url=*.$d&output=text&fl=original&collapse=urlkey`, `otx.alienvault.com/api/v1/indicators/domain/$d/passive_dns`, `urlscan.io/api/v1/search/?q=domain:$d`. Triage the result list for `dev-*`, `staging-*`, `beta`, `qa`, `uat`, `internal-*`, `*-preview` prefixes first — those are the weak-auth surface.
...
## DATACENTER-IP BLOCK DETECTION (don't mistake it for a finding)

Enterprise targets (New Relic, Cloudflare-fronted apps, edge-WAF stacks) silently blocklist VPS/datacenter IP ranges. The tell is an unusual status code with a plaintext block message. From a VPS probe, New Relic's staging/edge hosts returned:
...
A `418` (or sometimes `403`/`429`) carrying a literal `Your IP has been blocked` / `Access Denied` / `Sorry, you have been blocked` body across MULTIPLE unrelated subdomains is **environment state, not a target behavior** — do NOT report it, and do NOT waste time fingerprinting "what's behind the 418". It just means your egress IP is on a datacenter blocklist.

Implication for the recon→hunt handoff: any authenticated testing (NerdGraph/GraphQL mutations, IDOR, RBAC, post-login `return_to` open-redirect) must run from a **residential IP** — i.e. the user's own browser/session, or a residential proxy. Surface this to the user early as a blocker and pivot to: "you log in, hand me the session cookie/token, I drive the authenticated requests" rather than burning attempts from the blocked VPS. Cross-check: if `one.newrelic.com`-class hosts answer 200/302 normally but `staging-*` / `dev-*` return the block page, it's IP-based, not a per-host WAF rule.
