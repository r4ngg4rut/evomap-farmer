...
## Kill Signals (skip if present)
- TVL under $500K isn't worth the audit time unless the bounty floor is high
- Audit firm already covered it = low ROI unless you find what they missed — look at the audit-report scope-exclusion section for what they EXPLICITLY didn't audit (oracles, governance, off-chain components, frontend, the admin path)
- **30+ professional audits** (ChainSecurity, OZ, Trail of Bits) = extremely low ROI for individual hunters — only worth targeting newly added features (last 6-12 months) or audit scope exclusions
- Protocol is **mature + heavily forked** (e.g., Uniswap v2 forks) — bugs already found across ecosystem

## Protocol Maturity Assessment
Before deep-diving a smart contract target:
1. Count audit reports in `/audits/` directory — 10+ = mature, 30+ = extremely mature
2. Check audit dates — recent audits (last 6 months) mean fresh code was reviewed
3. Look for **external position types** or **adapter contracts** — each new type is separate attack surface, often less reviewed
4. Check **commit history** for recent additions — new features = new bugs
5. Read the **Known Risks & Mitigations** section in protocol docs — often reveals trust assumptions you can test

See `references/fork-testing-lessons.md` for practical Foundry fork testing patterns.
See `references/post-audit-delta-triage.md` for the git-churn workflow to audit heavily-audited protocols (3+ audits): un-shallow the clone, rank `.sol` files by changes since the newest audit date, triage top-churn files with the bug-class playbook, and report clean results honestly without inflating anomalies into findings. Validated against Curvance (6 audits, 431 post-audit commits).
